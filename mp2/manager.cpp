...
{	
	#if PRINT_INFO == 1
		cout << "debug: wait for convergence of " << topology.num_nodes << " nodes" << endl;
	#endif
 ...
 */
 void* input_thread_handle(void* data)
 {
 	while (true)
 	{
		...
		{
			Link* node = &topology.matrix[id][id];
			string text = "start messages";
			
			struct addrinfo* node_addr;
			getaddrinfo(node->ip.c_str(), node->port.c_str(), &hints, &node_addr);
			Utility::send(server_socket, text.c_str(), node_addr->ai_addr, node_addr->ai_addrlen);
		}
...
int main(int argc, char* argv[])
{
	if (argc != 3)
	{
		cout << "usage: ./manager [topology file] [message file]" << endl;
		return 0;
	}
	else 
	{
		cout << "Topology file: " << argv[1] << endl;
		cout << "Message file: " << argv[2] << endl;
	}
	
	// Get information about the topology
	topology.parse_topology_file(argv[1]);
	node_messages = Message::parse_message_file(argv[2]);
